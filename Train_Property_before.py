...
def main():
    ...
    parser.add_argument('--model_type', type=str, default='simple', help='model type: simple or test')
    ...
    for fold_idx, seed in enumerate(seeds):
        ...
        if args.project == '':
            args.project = f"{args.dataset.upper()}_{args.feature}_{args.model_type}"
        ...
        # dataset = MoleculeDataset("dataset/" + 'bace', dataset='bace')
        ...
        if args.feature == '2D-GNN':
            from process.graph_cl_models import GNN_graphpred
            model = GNN_graphpred(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type)
            if not args.input_model_file == "":
                model.from_pretrained(args.input_model_file)
        else:
            from model.experiment_model import Property_norm, Property_simple
            if args.model_type == 'norm':
                model = Property_norm(feature_type=args.feature, num_tasks=num_tasks)
            elif args.model_type == 'simple':
                model = Property_simple(feature_type=args.feature, num_tasks=num_tasks)
            else:
                raise ValueError(f"Invalid model_type: {args.model_type}. Must be 'norm' or 'simple'")
        
        # Check if model was successfully created
        if model is None:
            raise ValueError(f"Failed to create model with feature type: {args.feature}")
